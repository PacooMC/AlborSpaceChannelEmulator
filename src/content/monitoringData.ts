import { Scenario } from './scenarios'; // Assuming Scenario type is defined here or in App.tsx
    ...
    export const generateSignalMetrics = (scenarioId: string | null, prevMetrics?: SignalMetrics): SignalMetrics => {
      ...
      const idOffsetElevationPhase = getIdOffset(scenarioId, 100); // Offset for elevation oscillation
      ...
      // Simulate elevation angle oscillating (e.g., satellite pass)
      const elevationCenter = 45; // Center point of oscillation
      const elevationAmplitude = 35; // Amplitude (45 +/- 35 -> 10 to 80)
      const elevationPeriod = 60 * 5; // Period of oscillation in seconds (e.g., 5 minutes)
      const elevation = elevationCenter + elevationAmplitude * Math.sin(
          (simTime / 1000 + idOffsetElevationPhase) * (2 * Math.PI / elevationPeriod)
      );
      ...
      return {
        ...
        elevation: parseFloat(elevation.toFixed(1)), // Add elevation
      };
    };
