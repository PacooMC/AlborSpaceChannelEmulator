import React, { useState, useEffect, useCallback, useRef } from 'react';
        ...
        import { ScenarioNode, CustomNodeData, ScenarioType, KeplerianElements, MovementPatternType, MovementParameters, CircularPathParams, LinearMovementParams, PathBehavior } from './types'; // Updated import, added PathBehavior
        // Removed FlatMapPathEditor import from here
        import MovementPatternEditorModal from './MovementPatternEditorModal'; // Import the modal
        ...
        // --- Helper Components (ConfigInput, MapPreview, OrbitParamsDisplay, InfoBox - no changes needed) ---
        ...
        const NodeConfigPanel: React.FC<NodeConfigPanelProps> = ({ selectedNode, scenarioType, onNodeUpdate }) => {
          ...
          // Updated to show simplified movement pattern names and path info
          const renderMovementConfigSummary = () => {
            if (scenarioType !== 'custom') return null;
            const pattern = nodeData.movementPattern || 'STATIC';
            let summaryText = pattern === 'CIRCULAR_PATH' ? 'Circular Path' : pattern; // Display name
            let behaviorText = '';

            // Add specific details based on pattern
            if (pattern === 'CIRCULAR_PATH' && nodeData.movementParams) {
                const params = nodeData.movementParams as CircularPathParams;
                const details = [];
                if (params.startLon !== undefined && params.startLat !== undefined) details.push(`Start: ${params.startLat.toFixed(1)}°, ${params.startLon.toFixed(1)}°`);
                if (params.endLon !== undefined && params.endLat !== undefined) details.push(`End: ${params.endLat.toFixed(1)}°, ${params.endLon.toFixed(1)}°`);
                if (params.altitudeKm !== undefined) details.push(`Alt: ${params.altitudeKm}km`);
                if (details.length > 0) {
                    summaryText = `Circular Path (${details.join(' | ')})`;
                }
                if (params.pathBehavior === 'bounce') behaviorText = '(Bounce)';
            } else if (pattern === 'LINEAR' && nodeData.movementParams) {
                 const params = nodeData.movementParams as LinearMovementParams;
                 const details = [];
                 if (params.startLon !== undefined && params.startLat !== undefined) details.push(`Start: ${params.startLat.toFixed(1)}°, ${params.startLon.toFixed(1)}°`);
                 if (params.endLon !== undefined && params.endLat !== undefined) details.push(`End: ${params.endLat.toFixed(1)}°, ${params.endLon.toFixed(1)}°`);
                 if (params.speedKmh !== undefined) details.push(`Speed: ${params.speedKmh}km/h`);
                 if (details.length > 0) {
                     summaryText = `LINEAR (${details.join(' | ')})`;
                 } else if (params.speedKmh !== undefined) {
                     summaryText = `LINEAR (Speed: ${params.speedKmh}km/h)`;
                 }
                 if (params.pathBehavior === 'bounce') behaviorText = '(Bounce)';
            }
            ...
            return (
                <div className="mt-3">
                    ...
                    <p className="text-xs text-albor-light-gray bg-albor-deep-space/50 border border-albor-bg-dark rounded px-2 py-1 truncate" title={`${summaryText} ${behaviorText}`}>
                        {summaryText} <span className="text-albor-dark-gray">{behaviorText}</span>
                    </p>
                </div>
            );
          };
            ...
            <>
                ...
                {selectedNode && (
                    <MovementPatternEditorModal
                        ...
                        // Use the simplified pattern type
